...
// Get channels for a user in a specific group
...
router.post('/api/channels/join-chatroom', async (req, res) => {
  const { username, channelId } = req.body;
  try {
      // Update the channel to add the user
      await Channel.updateOne({ _id: channelId }, { $addToSet: { members: username } });

      // Optionally return the updated channel information
